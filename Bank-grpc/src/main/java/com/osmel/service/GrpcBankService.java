...
public class GrpcBankService extends BankServiceGrpc.BankServiceImplBase {
    ...
    public void getStream(Ebank.ConvertCurrencyRequest request, StreamObserver<Ebank.ConvertCurrencyResponse> responseObserver) {
        ...
        timer.schedule(new TimerTask() {
            ...
            @Override
            public void run() {
                Ebank.ConvertCurrencyResponse response= Ebank.ConvertCurrencyResponse.newBuilder().setCurrencyTo(request.getCurrencyTo()).setCurrencyFrom(request.getCurrencyFrom()).setAmount(amount)
    ...
    public StreamObserver<Ebank.ConvertCurrencyRequest> fullStream(StreamObserver<Ebank.ConvertCurrencyResponse> responseObserver) {
        return new StreamObserver<Ebank.ConvertCurrencyRequest>() {
            ...
            @Override
            public void onNext(Ebank.ConvertCurrencyRequest convertCurrencyRequest) {
                Ebank.ConvertCurrencyResponse response = Ebank.ConvertCurrencyResponse.newBuilder().setResult(convertCurrencyRequest.getAmount()*Math.random()*40).build();
                responseObserver.onNext(response);
            }
